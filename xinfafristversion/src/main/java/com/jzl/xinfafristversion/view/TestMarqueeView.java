...
/**
 * TODO 这里处理了跑马灯能显示outAnimation的效果，因此暂时不用MyMarqueeView和MarqueeView
 */
public class TestMarqueeView<T> extends ViewFlipper {
    private static final String TAG = "TestMarqueeView";
    ...
    private TestMarqueeView.OnItemClickListener onItemClickListener;

    public TestMarqueeView(Context context, MyBean.GroupsBean.GroupBean.AreasBean.AreaBean areaBean) {
        this(context, areaBean, null);

    }

    public TestMarqueeView(Context context, MyBean.GroupsBean.GroupBean.AreasBean.AreaBean areaBean, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void initData() {
        if (this.areaBean==null || this.areaBean.getFiles() == null || this.areaBean.getFiles().getFile() == null) {
            MyLogger.e(TAG,"initData areaBean property is null");
            return;
        }
    ...
    private void start(final @AnimRes int inAnimResId, final @AnimRes int outAnimResID) {
        ...
        position = 0;
        if (messages.size() > 1) {
            TextView one = createTextView();
            one.setTag(0);
            addView(one);
            TextView two = createTextView();
            two.setTag(1);
            addView(two);

//            CharSequence message1 = getMessageContent(0);
//            one.setText(message1);
//
//            CharSequence message2 = getMessageContent(1);
//            two.setText(message2);
        ...
        if (getInAnimation() != null) {
            getInAnimation().setAnimationListener(new Animation.AnimationListener() {
                ...
                public void onAnimationStart(Animation animation) {
                    ...
                    int whichTextView = position % getChildCount();
                    View view = getChildAt(whichTextView);
                    if (view instanceof TextView) {
                        T t = messages.get((position++) % messages.size());
                        ((TextView) view).setText(getMsgStr(t));
                    }
                }
    ...
    private CharSequence getMsgStr(T t){
        CharSequence message = "";
        if (t instanceof CharSequence) {
            message = (CharSequence) t;
        } else if (t instanceof IMarqueeItem) {
            message = ((IMarqueeItem) t).marqueeMessage();
        }
        return message;
    }

    private CharSequence getMessageContent(int position) {
        T msg = messages.get(position);
        CharSequence message = "";
        if (msg instanceof CharSequence) {
            message = (CharSequence) msg;
        } else if (msg instanceof IMarqueeItem) {
            message = ((IMarqueeItem) msg).marqueeMessage();
        }
        return message;
    }
...
//        textView.setLayoutParams(lp);
    ...
    public void setOnItemClickListener(TestMarqueeView.OnItemClickListener onItemClickListener) {
    ...
    public void startMarqueeView() {
        if (areaBean != null) {
            ArrayList<FileBean> fileBeanArrayList = areaBean.getFiles().getFile();
            if (areaBean.getFiles() == null || areaBean.getFiles().getFile()==null) {
                return;
            }
            //设置进场出场方式
            ...
            if (fileBeanArrayList.size() == 1) {
                T content = (T) fileBeanArrayList.get(0).getPath();
                list.add(content);
                list.add(content);
            } else if (fileBeanArrayList.size() > 1) {
                for (int i = 0; i < fileBeanArrayList.size(); i++) {
                    T c = (T) fileBeanArrayList.get(i).getPath();
                    list.add(c);
                }
            }
//            list.add((T) "锄禾日当午");
